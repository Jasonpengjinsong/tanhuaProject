...
public class MLlibRecommend {
    ...
    public MatrixFactorizationModel baseModel(JavaPairRDD<Long, Rating> ratings) {
        //统计用户数量和动态数量以及用户对动态的评分数目
        ...
        //训练集
        JavaRDD<Rating> training = ratings.filter(v -> v._1() < 6).values().repartition(numpartitions).cache();
        //校验集
        JavaRDD<Rating> validation = ratings.filter(v -> v._1() >= 6 && v._1() < 8).values().repartition(numpartitions).cache();
        //测试集
        JavaRDD<Rating> test = ratings.filter(v -> v._1() >= 8).values().cache();
